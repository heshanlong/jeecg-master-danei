...
public class SystemServiceImpl extends CommonServiceImpl implements SystemService {
	...
	private static  final DateFormat sdf2 = new SimpleDateFormat("yyyyMMdd HH:mm:ss");
...
//		log.setTSUser(ResourceUtil.getSessionUser());
	...
	@Override
	public void getFamsNumberByTSTypegroup(String tsTypeName,HttpServletRequest req) {
		String number;
		List<TSTypegroup> tSTypegroupList =super.findByProperty(TSTypegroup.class, "typegroupcode", "form_no");
		if (tSTypegroupList!=null && tSTypegroupList.size() > 0) {
			...
			for (int i = 0; i < tSTypeList.size(); i++) {
				if (!tSTypeList.get(i).getTypename().equals(tsTypeName)) {
					continue;
				}
				String[] strArry = tSTypeList.get(i).getTypecode().split("\\.");
				if (strArry.length > 1) {
					req.setAttribute("no", strArry[0] + new Date().getTime() + strArry[1]);
					break;
				}
			}
		}
	...
	@Override
	public String getFamsNumberByTSTypegroup(String tsTypeName, Class clazz, String property) {

		return SeqGenerator( tsTypeName,  clazz,property);
	}


	public String SeqGenerator(String tsTypeName, Class clazz, String property) {
		String leftNumber="";
		String returnCode = "";
		...
		try {
			...
			if (tSTypegroupList!=null && tSTypegroupList.size() > 0) {
				...
				for (int i = 0; i < tSTypeList.size(); i++) {
					if (tSTypeList.get(i).getTypename().equals(tsTypeName)) {
						...
						//数据字典配置值+当天年月日+数据库当天已有记录+1
						leftNumber=tSTypeList.get(i).getTypecode();
			...
			String key=leftNumber+dateString;
			String upperCode = "";
			Long size = redisTemplate.opsForList().size(key);//查找以prefix作为key值的数据长度

			if (size > 0) {//有数据
				List leve = redisTemplate.opsForList().range(key, 0, -1);//获取该key下面的所有值 （-1所有值；1下一个值）
			...
			returnCode =		key+ df.format(Suffix);//后缀不够numLength长，前面补充0

			redisTemplate.opsForList().rightPush(key, String.valueOf(Suffix));//存入Redis key（表单前缀+年月日），value(所属业务表当日已存多少条数据)
///		redisTemplate.opsForValue().set(key, String.valueOf(Suffix),60*60*24,TimeUnit.SECONDS);//问题代码
//		redisTemplate.expire(key,60*60*24,TimeUnit.SECONDS);//问题代码
          redisTemplate.expire(key,60*60*24,TimeUnit.SECONDS);//设置过期时间



		}
		//当redis服务出现问题，调用mysql的统计方法，保障可用性
		catch(RedisConnectionFailureException jedisConnectionException){
			returnCode=getFamsNumberString( tsTypeName,  clazz,  property);
	...
	private   String getFamsNumberString(String tsTypeName, Class clazz, String property) {
		String number="";
			...
			if (tSTypegroupList!=null && tSTypegroupList.size() > 0) {
				...
				for (int i = 0; i < tSTypeList.size(); i++) {
					if (tSTypeList.get(i).getTypename().equals(tsTypeName)) {
						...
						cri.add(Restrictions.between(property,sdf2.parse(dateString+" 00:00:00"),sdf2.parse(dateString+" 23:59:59")));
						List list = cri.list();
						//数据字典配置值+当天年月日+数据库当天已有记录+1
						number=tSTypeList.get(i).getTypecode()+dateString+df.format(list.size()+1);
	...
	@Override
	public String getFamsNumberByTSTypegroup(String tsTypeName, Class clazz, String property, String userName) {
			...
			if (tSTypegroupList!=null && tSTypegroupList.size() > 0) {
				...
				for (int i = 0; i < tSTypeList.size(); i++) {
					if (tSTypeList.get(i).getTypename().equals(tsTypeName)) {
						Session session = super.getSession();
						Criteria cri = session.createCriteria(clazz);
						cri.add(Restrictions.between(property,formatTime.parse(dateString+" 00:00:00"),formatTime.parse(dateString+" 23:59:59")));
						cri.add(Restrictions.eq("createBy", userName));
						List list = cri.list();
						//数据字典配置值+当天年月日+数据库当天已有记录+1
						number=tSTypeList.get(i).getTypecode()+dateString+numberFormat.format(list.size()+1);
						break;
					}
	...
	@Override
	public String getFamsNumberByTSTypegroupForApp(String tsTypeName) {
		String number=null;
		List<TSTypegroup> tSTypegroupList =super.findByProperty(TSTypegroup.class, "typegroupcode", "form_no");
		if (tSTypegroupList!=null && tSTypegroupList.size() > 0) {
			List<TSType> tSTypeList = tSTypegroupList.get(0).getTSTypes();
			for (int i = 0; i < tSTypeList.size(); i++) {
				...
				String[] strArry = tSTypeList.get(i).getTypecode().split("\\.");
				if (strArry.length > 1) {
					number=strArry[0] + new Date().getTime() + strArry[1];
					break;
				}
			}
		}
