...
import org.jeecgframework.web.system.dao.RoleDao;
...
//		log.setTSUser(ResourceUtil.getSessionUser());
    ...
    public void saveLoginUserInfo(HttpServletRequest req, TSUser user, String orgId) {
    	...
    	//mchen 登录成功，踢除相同用户名已登录的帐号
        try {
			new OnlineMessageManger().kickOutUser(user.getUserName());
		} catch (Exception e1) {
			e1.printStackTrace();
		}
	...
	@Override
	public String getMobilePhoenByUserId(String userId) {
		return roleDao.getMobilePhoneByUserId(userId);
	}
