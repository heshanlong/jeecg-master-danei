...
import org.jeecgframework.web.system.pojo.base.TSUuserDetail;
...
public class UserController extends BaseController {
	...
	public void interfaceUserDatagrid(TSUser user, HttpServletRequest request, HttpServletResponse response,
			DataGrid dataGrid) {
		...
		for (Object o : dataGrid.getResults()) {
			if (o instanceof TSUser) {
				...
				if (cfe.getId() != null && !"".equals(cfe.getId())) {
					List<InterroleUserEntity> roleUser = systemService.findByProperty(InterroleUserEntity.class,
							"TSUser.id", cfe.getId());
					if (roleUser.size() > 0) {
						String roleName = "";
...
//			}
	...
	public void datagridByOrgCode(TSUser user, HttpServletRequest request, HttpServletResponse response,
			DataGrid dataGrid) {
		...
		// 查询机构下用户列表
		MiniDaoPage<TSUser> list = departAuthGroupDao.getUserByDepartCode(dataGrid.getPage(), dataGrid.getRows(),
				orgCode, user);
	...
	public void customDatagrid(TSUser user, HttpServletRequest request, HttpServletResponse response,
			DataGrid dataGrid) {
		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
		// 查询条件组装器
		org.jeecgframework.core.extend.hqlsearch.HqlGenerateUtil.installHql(cq, user);
		...
		// 获取 当前组织机构的用户信息
		if (!org.apache.commons.lang3.StringUtils.isEmpty(departName)) {
			CriteriaQuery subCq = new CriteriaQuery(TSDepart.class);
			subCq.setProjection(Property.forName("id"));
			subCq.like("departname", "%"+departName+"%");
			subCq.add();
			cq.add(Property.forName("departid").in(subCq.getDetachedCriteria()));
		}
...
//		List<TSRoleUser> roleUser = systemService.findByProperty(TSRoleUser.class, "TSUser.id", user.getId());
		if (!user.getStatus().equals(Globals.User_ADMIN)) {
			...
			systemService.deleteAllEntitie(groupUsers);
			 ...
			 * 
			 * systemService.executeSql("delete from t_s_user_org where user_id=?",
			 * user.getId()); // 删除 用户-机构 数据
			 * 
			 * userService.delete(user); message = "用户：" + user.getUserName() + "删除成功";
			 * systemService.addLog(message, Globals.Log_Type_DEL, Globals.Log_Leavel_INFO);
			 * } else { userService.delete(user); message = "用户：" + user.getUserName() +
			 * "删除成功"; }
			 **/
		} else {
			message = "超级管理员不可删除";
	...
	public AjaxJson trueDel(TSUser user, HttpServletRequest req) {
		 ...
		 * "TSUser.id", user.getId()); if (!user.getStatus().equals(Globals.User_ADMIN))
		 * { if (roleUser.size()>0) { // 删除用户时先删除用户和角色关系表 delRoleUser(user);
		 * systemService.executeSql("delete from t_s_user_org where user_id=?",
		 * user.getId()); // 删除 用户-机构 数据 userService.delete(user); message = "用户：" +
		 * user.getUserName() + "删除成功"; systemService.addLog(message,
		 * Globals.Log_Type_DEL, Globals.Log_Leavel_INFO); } else {
		 * userService.delete(user); message = "用户：" + user.getUserName() + "删除成功"; } }
		 * else { message = "超级管理员不可删除"; }
		 */
	...
	/*
	 * public void delRoleUser(TSUser user) { // 同步删除用户角色关联表 List<TSRoleUser>
	 * roleUserList = systemService.findByProperty(TSRoleUser.class, "TSUser.id",
	 * user.getId()); if (roleUserList.size() >= 1) { for (TSRoleUser tRoleUser :
	 * roleUserList) { systemService.delete(tRoleUser); } } }
	 */
	/**
	 * 检查用户名
	 * 
	...
	public AjaxJson saveUser(HttpServletRequest req, TSUser user) {
		...
		if (StringUtil.isNotEmpty(user.getId())) {
			...
			if (users != null) {
				...
				// 添加t_s_user_detail
				TSUuserDetail tud = new TSUuserDetail();
				tud.setId(user.getId());
				userService.save(tud);
			}
		}
...
//			}
	...
	public String exportXls(TSUser tsUser, HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid,
		...
		for (int i = 0; i < tsUsers.size(); i++) {
			...
			List<TSRole> roles = systemService.getSession().createSQLQuery(queryRole).addEntity(TSRole.class)
					.setString("userid", id).list();
			String roleCodes = "";
			...
			List<TSDepart> departs = systemService.getSession().createSQLQuery(queryDept).addEntity(TSDepart.class)
					.setString("userid", id).list();
			String departCodes = "";
		...
		modelMap.put(NormalExcelConstants.PARAMS,
				new ExportParams("用户表列表", "导出人:" + ResourceUtil.getSessionUser().getRealName(), "导出信息"));
		...
		for (Map.Entry<String, MultipartFile> entity : fileMap.entrySet()) {
			...
			try {
				...
				for (TSUser tsUser : tsUsers) {
					...
					tsUser.setPassword(PasswordUtil.encrypt(username, "888888", PasswordUtil.getStaticSalt()));
					...
					if ((roleCodes == null || roleCodes.equals("")) || (deptCodes == null || deptCodes.equals(""))) {
						...
						for (String deptCode : depts) {
							List<TSDepart> departList = systemService.findByProperty(TSDepart.class, "orgCode",
									deptCode);
						...
						if (flag) {
							...
							if (users.size() != 0) {
								...
								for (String roleCode : roles) {
									// 根据角色编码得到roleid
									List<TSRole> roleList = systemService.findByProperty(TSRole.class, "roleCode",
											roleCode);
