...
//				|| 
		if (key.equals(DataBaseConstant.SYS_USER_CODE)
			...
			returnValue = (getSessionUser()==null? SessionUserUtil.getCurrentUser().getUser():ResourceUtil.getSessionUser()).getUserName();
		}
		//替换为系统登录用户真实名字
...
//				|| key.equals(DataBaseConstant.UPDATE_NAME)
		if (key.equals(DataBaseConstant.SYS_USER_NAME)
			...
			returnValue =  (getSessionUser()==null? SessionUserUtil.getCurrentUser().getUser():ResourceUtil.getSessionUser()).getRealName();
		}

		//替换为系统登录用户的公司编码
		if (key.equals(DataBaseConstant.SYS_COMPANY_CODE)|| key.equals(DataBaseConstant.SYS_COMPANY_CODE_TABLE)) {

			returnValue = (getSessionUser()==null? SessionUserUtil.getCurrentUser().getUser():ResourceUtil.getSessionUser()).getCurrentDepart().getOrgCode()
