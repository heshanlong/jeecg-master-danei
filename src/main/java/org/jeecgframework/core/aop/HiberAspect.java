...
public boolean onSave(Object entity, Serializable id, Object[] state,
	...
	try {
		//邓正辉，在此添加支持app端创建人，创建时间等字段aop
		currentUser = ResourceUtil.getSessionUser()==null? SessionUserUtil.getCurrentUser().getUser():ResourceUtil.getSessionUser();
	...
	try {
		//添加数据
		 ...
		 {
			 /*找到名为"创建用户id"的属性*/
			 if (DataBaseConstant.CREATE_USER_ID.equals(propertyNames[index]))
			 {
				 /*使用拦截器将对象的"创建用户id"属性赋上值*/
				 if(oConvertUtils.isEmpty(state[index])){
					 state[index] = Util.getPcOrAppUserId();
				 }
				 continue;
			 }
		     /*找到名为"创建时间"的属性*/
