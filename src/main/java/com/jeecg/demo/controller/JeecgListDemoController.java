...
//					+ "";//TODO keyword 没用到
			String sql = "select   @rownum := @rownum +1 AS rownum,a.username,a.realname,c.departname as depart from   (SELECT    @rownum := 0) r,t_s_base_user a left join t_s_user_org b on b.user_id  = a.ID left join t_s_depart c on c.id = b.org_id where c.departname like "+" '%"+keyword+"%' ";
...
//						data.get(i).put("rownum",i);
//						if(null == data.get(i).get(key)){
//							data.get(i).put(key,"");
//						}
//					}
//				}
