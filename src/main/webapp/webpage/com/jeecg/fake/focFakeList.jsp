...
<body>
	<div class="panel-body" style="padding-top: 0px; padding-bottom: 0px;">
		<!-- toolbar -->
		<div id="toolbar">
			<button onclick="add('录入','focFakeController.do?goAdd','focFakeList',600,400)" id="btn_add" class="btn btn-primary btn-sm">
				<span class="glyphicon glyphicon-plus" aria-hidden="true"></span> 录入
			</button>
			<button onclick="update('编辑','focFakeController.do?goUpdate','focFakeList',600,400)" id="btn_edit" class="btn btn-success btn-sm">
				<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span> 编辑
			</button>
			<button onclick="deleteALLSelect('批量删除','focFakeController.do?doBatchDel','focFakeList',600,400)" id="btn_delete" class="btn btn-danger btn-sm">
				<span class="glyphicon glyphicon-remove" aria-hidden="true"></span> 批量删除
			</button>
			<button onclick="detail('查看','focFakeController.do?goUpdate','focFakeList',600,400)" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-search" aria-hidden="true"></span> 查看
			</button>
			<button onclick="ImportXls('导入','','focFakeList',600,400)" id="btn_download" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-download" aria-hidden="true"></span> 导入
			</button>
			<button onclick="ExportXls('导出','','focFakeList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
				<span class="glyphicon glyphicon-cloud-upload" aria-hidden="true"></span> 导出
			</button>
			<button onclick="ExportXlsByT('模板下载','','focFakeList',600,400)" id="btn_upload" class="btn btn-info btn-sm">
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			$('#focFakeList').on(
					...
					function() {
						$('#btn_delete').prop('disabled',!$('#focFakeList').bootstrapTable('getSelections').length);
						$('#btn_edit').prop('disabled',$('#focFakeList').bootstrapTable('getSelections').length != 1);
					});
		...
		var TableInit = function() {
			...
			oTableInit.Init = function() {
				$('#focFakeList').bootstrapTable({
									url : 'focFakeController.do?datagrid', //请求后台的URL（*）
									method : 'get', //请求方式（*）
									...
									columns : [
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var pageSize = $('#focFakeList')
															.bootstrapTable('getOptions').pageSize;
													var pageNumber = $('#focFakeList').bootstrapTable('getOptions').pageNumber;
											...
											},
											{
												...
												formatter : function(value,row, index) {
													...
													var href = '';
													href += "<a href='javascript:void(0);'  class='ace_button'  onclick=delObj('focFakeController.do?doDel&id="
															+ row.id
			...
			oTableInit.queryParams = function(params) {
				var temp = { //这里的键的名字和控制器的变量名必须一直，这边改动，控制器也需要改成一样的
					...
					field : 'id,createName,createBy,createDate,updateName,updateBy,updateDate,sysOrgCode,sysCompanyCode,bpmStatus,aaa,bbb,ccc,'
				};
		...
		function reloadTable() {
			$('#focFakeList').bootstrapTable('refresh');
		}
	...
	<script type="text/javascript">
		...
		//导入
		function ImportXls() {
			openuploadwin('Excel导入', 'focFakeController.do?upload',"focFakeList");
		}

		//导出
		function ExportXls() {
			JeecgExcelExport("focFakeController.do?exportXls","focFakeList");
		}

		//模板下载
		function ExportXlsByT() {
			JeecgExcelExport("focFakeController.do?exportXlsByT","focFakeList");
		}
		...
		function listDictFormat(value,code,table){
			...
			var dicts = focFakeListdictsData[dictKey];
		...
		function loadSearchFormDicts(obj,table,code,type,name){
			...
			var arr = focFakeListdictsData[dictKey];
