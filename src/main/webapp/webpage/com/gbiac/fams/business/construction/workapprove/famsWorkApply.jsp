...
<t:datagrid name="famsWorkApproveList" component="bootstrap-table" checkbox="true" sortName="createDate"
            ...
            actionUrl="famsWorkApproveController.do?apply" idField="id" fit="true" queryMode="group">
    ...
    <t:dgCol title="作业申报单位"  field="workDepart" query="true" hidden="false" queryMode="single"  width="120"></t:dgCol>
    <t:dgCol title="作业申报单位id"  field="workApproveDepart"  hidden="true" queryMode="single"  width="120"></t:dgCol>
    ...
    <t:dgToolBar title="录入" icon="icon-add" url="famsWorkApproveController.do?goAdd" funname="addRewrite" width="100%" height="100%"></t:dgToolBar>
    <t:dgToolBar title="重新提交" icon="icon-repeat" url="famsWorkApproveController.do?goUpdateForApplyAgain" funname="updateRewrite" width="100%" height="100%"></t:dgToolBar>
    <t:dgToolBar title="撤回" icon="icon-redo" url="" funname="undo"  width="100%" height="100%"></t:dgToolBar>
    ...
    <t:dgToolBar title="流程图预览" icon="icon-search" url="famsWorkApproveController.do?viewBMP" funname="add"  width="100%" height="100%"></t:dgToolBar>
</t:datagrid>
<script type="text/javascript">
	...
	function jumpBMPPage(id, flag, funname, taskkey, jumpurl, taskname,
			createName, getPcOrAppUserName, getWorkStartTime, getWorkEndTime) {
		//  date为现在时间，与施工开始时间和施工结束时间比较
		var date = new Date();
		var date1 = new Date(getWorkStartTime);
		var date2 = new Date(getWorkEndTime);
		var str = "提交申请";
		var str1 = "申请进场";
		var str2 = "申请离场";
		//根据任务状态进行判断
		if (taskname == str) { //提交申请时-对提交人进行判断
			if (createName == getPcOrAppUserName) {
				mycreatedetailwindow(taskname,
						"famsWorkApproveController.do?jumpBMPPage&id=" + id
								+ "&flag=" + flag + "&funname=" + funname
								+ "&taskkey=" + taskkey + "&jumpurl=" + jumpurl
								+ "&taskname=" + taskname + "", '100%', '100%');
			} else {
				layer.confirm('当前作业申请非本人操作,是否继续提交？', {
					btn : [ '是', '否' ]
				}, function() {
					mycreatedetailwindow(taskname,
							"famsWorkApproveController.do?jumpBMPPage&id=" + id
									+ "&flag=" + flag + "&funname=" + funname
									+ "&taskkey=" + taskkey + "&jumpurl="
									+ jumpurl + "&taskname=" + taskname + "",
							'100%', '100%');
				}, function() {
					return;
				})
			}
		} else if (taskname == str1) {//申请进场-是否提前进场
			if (date < date1) {
				layer.confirm('当前未到进场时间，是否提前进场？', {
					btn : [ '是', '否' ]
				}, function() {
					mycreatedetailwindow(taskname,
							"famsWorkApproveController.do?jumpBMPPage&id="
									+ id + "&flag=" + flag
									+ "&funname=" + funname
									+ "&taskkey=" + taskkey
									+ "&jumpurl=" + jumpurl
		...
		} else if (taskname == str2) {//申请离场-对提前离场进行判断
			if(date <date2){
				layer.confirm('当前未到离场时间，是否提前离场？', {
					btn : [ '是', '否' ]
				}, function() {
					mycreatedetailwindow(taskname,
							"famsWorkApproveController.do?jumpBMPPage&id="
					...
					button : [
							{
								...
								callback : function() {
									...
									if (window.localStorage
											.getItem("uploadSuccess") != undefined) {
										var uploadSuccess = window.localStorage
									...
									window.localStorage
											.removeItem("uploadSuccess");

									$('#formobj', iframe.document)
											.attr('action',
													'famsWorkApproveController.do?doAdd&comfrom=submit');
									saveObj();
									return false;
								},
								focus : true
							},
							{
								...
								callback : function() {
									...
									$('#formobj', iframe.document)
											.attr('action',
													'famsWorkApproveController.do?doAdd');
									saveObj();
									return false;
								},
								focus : false
