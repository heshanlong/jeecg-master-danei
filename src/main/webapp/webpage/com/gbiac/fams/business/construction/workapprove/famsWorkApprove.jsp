...
<t:datagrid name="famsWorkApproveList" component="bootstrap-table" checkbox="true" sortName="createDate"
            ...
            actionUrl="famsWorkApproveController.do?approve" idField="id" fit="true" queryMode="group">
...
<script type="text/javascript">
	...
	function myApplyAgain(id) {
		createwindow("重新申请进场",
				"famsWorkApproveController.do?goUpdateForApplyAgain&id=" + id,
				"100%", "100%");
	}
	//操作栏中的撤回提交作业申请方法
	function undo(title, url, id, width, height, isRestful) {
		...
		$.ajax({
			...
			success : function(result) {
				if (result.success) {
					jumpBMPPage(rowsData[0].id, 's_undo_flag', 's_undo_fun',
							's_undo', 'flow/s-undo/page', '撤回');
				} else {
					tip(result.msg);
				}
			}
	...
	function myUpdate(id) {
		createwindow("编辑", "famsWorkApproveController.do?goUpdate&id=" + id,
				"100%", "100%");
	}
	//导入
	function ImportXls() {
		openuploadwin('Excel导入', 'famsWorkApproveController.do?upload',
				"famsWorkApproveList");
	}

	//导出
	function ExportXls() {
		JeecgExcelExport("famsWorkApproveController.do?exportXls",
				"famsWorkApproveList");
	}

	//模板下载
	function ExportXlsByT() {
		JeecgExcelExport("famsWorkApproveController.do?exportXlsByT",
				"famsWorkApproveList");
	}
	...
	function jumpBMPPage(id, flag, funname, taskkey, jumpurl, taskname) {
		mycreatedetailwindow(taskname,
				"famsWorkApproveController.do?jumpBMPPage&id=" + id + "&flag="
						+ flag + "&funname=" + funname + "&taskkey=" + taskkey
						+ "&jumpurl=" + jumpurl + "&taskname=" + taskname + "",
				'100%', '100%');
	}
