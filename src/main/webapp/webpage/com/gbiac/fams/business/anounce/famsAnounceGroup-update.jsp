...
<body style="overflow: hidden; overflow-y: auto;">
	<div class="container" style="width: 100%;">
		...
		<div class="panel-body">
			<form class="form-horizontal" role="form" id="formobj"
				action="famsAnounceGroupController.do?doUpdate" method="POST">
				...
				<div class="form-group">
					...
					<div class="col-sm-7">
						<div class="input-group" style="width: 100%">
							<input id="name" name="name" type="text"  value="${name}" maxlength="32" tiptype="6" datatype="s2-32" errormsg="名称必须是2-32的字符"
					...
					<div class="col-sm-7">
						<div class="input-group" style="width: 100%">
							...
							<a href="#"
								...
								id="departRedo" onclick="clears()">清空</a></c:if>
							<div
								style="font-size: 14px; display: inline-block;<c:if test="${empty load}"> margin-left: 30px;</c:if>">
								已选择 <label id="numbers">${count}</label>人
							</div>
						</div>
					</div>
				</div>
	...
	function callbackDepartmentSelect() {
        ...
        $('#realNames').val(realNamesArr.join(','));
        //显示在输入框上
    ...
    function check_val(str){
		...
		$(document).ready(function() {
			...
			$('#name').on('change',function () {
				var value = $(this).val();
				if(value !== '' && nameVal !== value
					&& check_val(value)){ //与名称不一致则请求
					console.log('进来请求');
					$.ajax({
						type: "get",
						dataType: "json",
				...
				.Validform(
						{
							...
							beforeSubmit : function(curform) {
								//分组名称判断是否重复
								console.log('beforeSubmit',submitFlag);
								if(!submitFlag){
									layer.msg('该分组名称重复');
									return false;
								}
